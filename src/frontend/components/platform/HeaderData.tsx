...
export function HeaderData({ tenantName, plan, tenantSlug }: Props) {
  ...
  useEffect(() => {
    fetch('/api/auth/me')
      .then((r) => r.json())
      .then((d) => { if (d.success) setUserName(d.data?.name || 'Admin') })
      .catch(() => {})
  }, [])
      ...
      <div className="flex items-center justify-between px-4 py-3 lg:px-6">
        <div className="flex items-center gap-3 pl-12 lg:pl-0">
          <div className="hidden lg:block">
            <h1 className="text-lg font-bold text-gray-800">{tenantName || 'Meu Restaurante'}</h1>
          ...
          {userMenuOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border py-1 animate-fade-in">
              <div className="px-4 py-2 border-b">
                ...
                <p className="text-xs text-gray-500">{tenantName || 'Restaurante'}</p>
              </div>
              <button
